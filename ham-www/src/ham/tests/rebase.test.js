...
describe("rebase_tests", () => {
  ...
  describe("rebase", () => {
    ...
    test("positive rebasing", async () => {
      ...
      let res_bal = await ham.contracts.yam.methods.balanceOf(
    ...
    test("no rebasing", async () => {
      ...
      // no change
      expect(ham.toBigN(bal1).toNumber()).toBe(yam.toBigN(bal).toNumber());
      // no increases to reserves
      expect(ham.toBigN(resHAM).toNumber()).toBe(0);
      expect(ham.toBigN(resycrv).toNumber()).toBe(0);
      r = await ham.contracts.uni_pair.methods.getReserves().call();
    ...
    test("rebasing with HAM in reserves", async () => {
      await ham.contracts.yam.methods.transfer(yam.contracts.reserves.options.address, yam.toBigN(60000*10**18).toString()).send({from: user});
      await ham.contracts.yam.methods.approve(
        ham.contracts.uni_router.options.address,
        -1
      ).send({
  ...
  describe("failing", () => {
    test("unitialized rebasing", async () => {
      await ham.testing.expectThrow(yam.contracts.rebaser.methods.activate_rebasing().send({
        from: user,
        gas: 500000
      }), "twap wasnt intitiated, call init_twap()");
    });
    test("no early twap", async () => {
      await ham.testing.expectThrow(yam.contracts.rebaser.methods.init_twap().send({
        from: user,
        gas: 500000
      }), "");
    });
    test("too late rebasing", async () => {
      ...
      await ham.testing.increaseTime(i + yam.toBigN(len).toNumber()+1);

      let b = await ham.testing.expectThrow(yam.contracts.rebaser.methods.rebase().send({
        from: user,
        gas: 2500000
      }), "too late");
    });
