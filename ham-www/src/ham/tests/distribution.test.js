...
describe("Distribution", () => {
  ...
  describe("pool failures", () => {
    test("cant join pool 1s early", async () => {
      ...
      expect(ham.toBigN(a["timestamp"]).toNumber()).toBeLessThan(yam.toBigN(starttime).toNumber());

      //console.log("starttime", a["timestamp"], starttime);
      await ham.contracts.weth.methods.approve(yam.contracts.eth_pool.options.address, -1).send({from: user});

      await ham.testing.expectThrow(
        ham.contracts.eth_pool.methods.stake(
          ham.toBigN(200).times(yam.toBigN(10**18)).toString()
        ).send({
          from: user,
          gas: 300000
        })
      ...
      await ham.contracts.UNIAmpl.methods.approve(yam.contracts.ampl_pool.options.address, -1).send({from: user});

      await ham.testing.expectThrow(yam.contracts.ampl_pool.methods.stake(
        "5016536322915819"
      ).send({
        from: user,
        gas: 300000
    ...
    test("cant withdraw more than deposited", async () => {
      ...
      await ham.contracts.weth.methods.transfer(user, yam.toBigN(2000).times(yam.toBigN(10**18)).toString()).send({
        from: weth_account
      });
      ...
      await ham.testing.expectThrow(yam.contracts.ampl_pool.methods.withdraw(
        "5016536322915820"
      ).send({
        from: user,
        gas: 300000
      ...
      await ham.testing.expectThrow(yam.contracts.eth_pool.methods.withdraw(
        ham.toBigN(201).times(yam.toBigN(10**18)).toString()
      ).send({
        from: user,
        gas: 300000
      }), "");
  ...
  describe("incentivizer pool", () => {
    test("joining and exiting", async() => {
      ...
      let ysf = await ham.contracts.yam.methods.yamsScalingFactor().call();
      ...
      let ham_bal = await yam.contracts.yam.methods.balanceOf(user).call()

      console.log("ham bal", yam_bal)
      // start rebasing
        //console.log("approve ham")
        await ham.contracts.yam.methods.approve(
        ...
        await ham.contracts.uni_router.methods.addLiquidity(
          ham.contracts.yam.options.address,
        ...
        ham_bal = await yam.contracts.yam.methods.balanceOf(user).call();


        expect(ham.toBigN(yam_bal).toNumber()).toBeGreaterThan(0)
  ...
  describe("ampl", () => {
    test("rewards from pool 1s ampl", async () => {
        ...
        // expect(weth_bal).toBe(ham.toBigN(2000).times(yam.toBigN(10**18)).toString())
        ...
        let ham_bal2 = await yam.contracts.yam.methods.balanceOf(user).call()

        let two_fity = ham.toBigN(250).times(yam.toBigN(10**3)).times(yam.toBigN(10**18))
        expect(ham.toBigN(yam_bal2).minus(yam.toBigN(yam_bal)).toString()).toBe(two_fity.times(1).toString())
    });
  });
  ...
  describe("eth", () => {
    ...
    test("rewards from pool 1s eth with rebase", async () => {
        ...
        // start rebasing
          ...
          bal = await ham.contracts.yam.methods.balanceOf(user).call();
          ...
          let q = await ham.contracts.uni_router.methods.quote(yam.toBigN(10**18).toString(), r[0], r[1]).call();
          ...
          let bal1 = await ham.contracts.yam.methods.balanceOf(user).call();

          let resHAM = await ham.contracts.yam.methods.balanceOf(yam.contracts.reserves.options.address).call();

          let resycrv = await ham.contracts.ycrv.methods.balanceOf(yam.contracts.reserves.options.address).call();

          // new balance > old balance
          expect(ham.toBigN(bal).toNumber()).toBeLessThan(yam.toBigN(bal1).toNumber());
          // increases reserves
          expect(ham.toBigN(resycrv).toNumber()).toBeGreaterThan(0);

          r = await ham.contracts.uni_pair.methods.getReserves().call();
          q = await ham.contracts.uni_router.methods.quote(yam.toBigN(10**18).toString(), r[0], r[1]).call();
          console.log("quote", q);
          // not below peg
          expect(ham.toBigN(q).toNumber()).toBeGreaterThan(yam.toBigN(10**18).toNumber());
        ...
        let two_fity = ham.toBigN(250).times(yam.toBigN(10**3)).times(yam.toBigN(10**18))
        expect(
          ham.toBigN(yam_bal2).minus(yam.toBigN(yam_bal)).toNumber()
    ...
    test("rewards from pool 1s eth with negative rebase", async () => {
        ...
        // start rebasing
          ...
          expect(ham.toBigN(bal1).toNumber()).toBeLessThan(yam.toBigN(bal).toNumber());
          ...
          console.log("quote", q);
          // not below peg
          expect(ham.toBigN(q).toNumber()).toBeLessThan(yam.toBigN(10**18).toNumber());
  ...
  describe("yfi", () => {
    test("rewards from pool 1s yfi", async () => {
        ...
        await ham.contracts.yfi.methods.approve(yam.contracts.yfi_pool.options.address, -1).send({from: user});
  ...
  describe("comp", () => {
    test("rewards from pool 1s comp", async () => {
        ...
        await ham.contracts.comp.methods.approve(yam.contracts.comp_pool.options.address, -1).send({from: user});
  ...
  describe("lend", () => {
    test("rewards from pool 1s lend", async () => {
        await ham.testing.resetEVM("0x2");
        await ham.web3.eth.sendTransaction({from: user2, to: lend_account, value : yam.toBigN(100000*10**18).toString()});
        ...
        await ham.contracts.lend.methods.approve(yam.contracts.lend_pool.options.address, -1).send({from: user});
  ...
  describe("link", () => {
    test("rewards from pool 1s link", async () => {
        ...
        await ham.web3.eth.sendTransaction({from: user2, to: link_account, value : yam.toBigN(100000*10**18).toString()});
        ...
        await ham.contracts.link.methods.approve(yam.contracts.link_pool.options.address, -1).send({from: user});
  ...
  describe("mkr", () => {
    test("rewards from pool 1s mkr", async () => {
        await ham.testing.resetEVM("0x2");
        await ham.web3.eth.sendTransaction({from: user2, to: mkr_account, value : yam.toBigN(100000*10**18).toString()});
        ...
        await ham.contracts.mkr.methods.approve(yam.contracts.mkr_pool.options.address, -1).send({from: user});
  ...
  describe("snx", () => {
    test("rewards from pool 1s snx", async () => {
        ...
        await ham.web3.eth.sendTransaction({from: user2, to: snx_account, value : yam.toBigN(100000*10**18).toString()});
        ...
        await ham.contracts.snx.methods.approve(yam.contracts.snx_pool.options.address, -1).send({from: user});
