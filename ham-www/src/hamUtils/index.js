...
export const getEarned = async (ham, pool, account) => {
  const scalingFactor = new BigNumber(await ham.contracts.yam.methods.yamsScalingFactor().call())
...
export const getCurrentPrice = async (ham) => {
  ...
  return ham.toBigN(await yam.contracts.rebaser.methods.getCurrentTWAP().call())
}
...
export const getCirculatingSupply = async (ham) => {
  ...
  let scalingFactor = ham.toBigN(await yam.contracts.yam.methods.yamsScalingFactor().call());
  let starttime = ham.toBigN(await yam.contracts.eth_pool.methods.starttime().call()).toNumber();
  let timePassed = now["timestamp"] - starttime;
  if (timePassed < 0) {
    return 0;
  }
  let hamsDistributed = yam.toBigN(8 * timePassed * 250000 / 625000); //yams from first 8 pools
  let starttimePool2 = ham.toBigN(await yam.contracts.ycrv_pool.methods.starttime().call()).toNumber();
  timePassed = now["timestamp"] - starttime;
  let pool2Hams = ham.toBigN(timePassed * 1500000 / 625000); // yams from second pool. note: just accounts for first week
  let circulating = pool2Yams.plus(hamsDistributed).times(scalingFactor).div(10**36).toFixed(2)
  return circulating
}
...
export const getNextRebaseTimestamp = async (ham) => {
  try {
    ...
    if (await ham.contracts.rebaser.methods.rebasingActive().call()) {
      ...
      let twap_init = ham.toBigN(await yam.contracts.rebaser.methods.timeOfTWAPInit().call()).toNumber();
      if (twap_init > 0) {
        let delay = ham.toBigN(await yam.contracts.rebaser.methods.rebaseDelay().call()).toNumber();
...
export const getTotalSupply = async (ham) => {
  return await ham.contracts.yam.methods.totalSupply().call();
}
...
export const delegate = async (ham, account) => {
  return ham.contracts.yam.methods.delegate("0x683A78bA1f6b25E29fbBC9Cd1BFA29A51520De84").send({from: account, gas: 320000 })
}

export const didDelegate = async (ham, account) => {
  return await ham.contracts.yam.methods.delegates(account).call() === '0x683A78bA1f6b25E29fbBC9Cd1BFA29A51520De84'
}

export const getVotes = async (ham) => {
  const votesRaw = new BigNumber(await ham.contracts.yam.methods.getCurrentVotes("0x683A78bA1f6b25E29fbBC9Cd1BFA29A51520De84").call()).div(10**24)
  return votesRaw
}

export const getScalingFactor = async (ham) => {
  return new BigNumber(await ham.contracts.yam.methods.yamsScalingFactor().call()).dividedBy(new BigNumber(10).pow(18))
}

export const getDelegatedBalance = async (ham, account) => {
  return new BigNumber(await ham.contracts.yam.methods.balanceOfUnderlying(account).call()).div(10**24)
}
