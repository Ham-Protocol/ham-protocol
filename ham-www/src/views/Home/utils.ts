...
const getCurrentPrice = async (ham: typeof Yam): Promise<number> => {
  // FORBROCK: get current HAM price
  return gCP(ham)
}

const getTargetPrice = async (ham: typeof Yam): Promise<number> => {
  // FORBROCK: get target HAM price
  return gTP(ham)
}

const getCirculatingSupply = async (ham: typeof Yam): Promise<string> => {
  // FORBROCK: get circulating supply
  return gCS(ham)
}

const getNextRebaseTimestamp = async (ham: typeof Yam): Promise<number> => {
  // FORBROCK: get next rebase timestamp
  const nextRebase = await gNRT(ham) as number
  return nextRebase * 1000
}

const getTotalSupply = async (ham: typeof Yam): Promise<string> => {
  // FORBROCK: get total supply
  return gTS(ham)
}

export const getStats = async (ham: typeof Yam) => {
